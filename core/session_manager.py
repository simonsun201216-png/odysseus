...
class SessionManager:
    ...
    def get_sessions_for_user(self, username: Optional[str] = None) -> Dict[str, Session]:
        ...
        NULL-owner sessions (legacy / pre-owner-tracking) are treated as
        shared — visible to any authenticated user. Only sessions that
        explicitly belong to a *different* user are hidden.
        """
        if username is None:
