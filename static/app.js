...
import casesModule from './js/cases.js';
import globalSearchModule from './js/global-search.js';
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Prophetis Chat';
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message Prophetis...');
      }
...
function startOdysseusApp() {
  ...
  // ── Bootstrap: single aggregated call replaces auth/status + features + settings ──
  // Fire immediately; stash features + settings into sessionStorage so the existing
  // init code (which checks sessionStorage first) picks them up without network.
  fetch(`${API_BASE}/api/bootstrap`, { credentials: 'same-origin' })
    .then(function(r) { return r.json(); })
    .then(function(data) {
      if (data.features) {
        try { sessionStorage.setItem('ody-prefetch-features', JSON.stringify(data.features)); } catch (_) {}
      }
      if (data.settings) {
        try { sessionStorage.setItem('ody-prefetch-settings', JSON.stringify(data.settings)); } catch (_) {}
      }
    })
    .catch(function() {});
  ...
  // Initialize global search (Ctrl+K unified omnibox)
  if (globalSearchModule) {
    globalSearchModule.init(API_BASE);
  }
  ...
  if (railSearchBtn) {
    railSearchBtn.addEventListener('click', () => {
      if (globalSearchModule) globalSearchModule.openSearch();
    });
  }
  ...
  const _railToolMap = {
    ...
    // rail-kb handled directly by kbPanel.js initKbPanel
  };
  ...
  // Load initial data — capture promise so we can coordinate loader removal
  const _presetsPromise = presetsModule.loadPresets(uiModule.showError);
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        // Wait for presets customization too, so the user never sees the
        // default layout flash before customization applies. Both run in
        // parallel — _presetsPromise was captured at line ~3897.
        Promise.resolve(_presetsPromise).then(() => {
          const loader = document.getElementById('app-loader');
          if (loader) { loader.style.opacity = '0'; setTimeout(() => loader.remove(), 300); }
        }).catch(() => {
          // Even if presets fail, still remove loader
          const loader = document.getElementById('app-loader');
          if (loader) { loader.style.opacity = '0'; setTimeout(() => loader.remove(), 300); }
        });
  ...
  // KB features - dynamic import to avoid blocking app.js module compilation
  import('./js/meetingDetail.js').then(function(md) { md.initMeetingDetail(); console.log('[kb] meetingDetail inited'); }).catch(function(e) { console.warn('[kb] meetingDetail import failed:', e); });
  import('./js/kbPanel.js').then(function(kb) { kb.initKbPanel(); window._openKbPanel = kb.openKbPanel; console.log('[kb] kbPanel inited, _openKbPanel ready'); }).catch(function(e) { console.warn('[kb] kbPanel dynamic import failed:', e); });

  // Non-critical lazy-loaded modules (fire-and-forget — don't block init)
  import('./js/assistant.js').catch(function(e) { console.warn('[lazy] assistant.js import failed:', e); });
  import('./js/tourHints.js').catch(function(e) { console.warn('[lazy] tourHints.js import failed:', e); });
  import('./js/tourAutoplay.js').catch(function(e) { console.warn('[lazy] tourAutoplay.js import failed:', e); });
  // cookbookSchedule.js is a non-module script — load via injected <script>
  (function() {
    var s = document.createElement('script');
    s.src = '/static/js/cookbookSchedule.js';
    s.async = true;
    document.body.appendChild(s);
  })();

  // Re-enable sidebar/icon-rail that were hidden by .prophetis-embed to
  // prevent flash of default layout before customization applied. This runs
  // after the full init sequence (sessions, presets, models, etc.) is complete,
  // so the user sees the fully customized view immediately.
  document.documentElement.classList.remove('prophetis-embed');
}
