...
# Throttle incognito-ghost cleanup so it runs at most once per 60s instead
# of on every /api/sessions request. The cleanup only catches edge cases
# (tab-close / crash orphans); frequent runs waste DB round-trips.
_last_incognito_cleanup_time = 0.0
_INCOGNITO_CLEANUP_INTERVAL = 60.0
...
def _verify_session_owner(request: Request, session_id: str, session_manager=None):
    ...
    if not user:
        if _auth_disabled():
            return  # Single-user mode — skip ownership check
...
def setup_session_routes(session_manager: SessionManager, config: dict, webhook_manager=None):
    ...
    def list_sessions(request: Request):
        global _last_incognito_cleanup_time
        user = effective_user(request)
        # Lazy purge (throttled): incognito sessions are ephemeral by design —
        # wipe leftovers from the DB and session_manager so they vanish on the
        # next page refresh. BUT: skip sessions that were created within the
        # last 10 minutes. Without that guard, the purge nukes the active
        # "Nobody" session on the very first /api/sessions call after creation,
        # killing the in-flight chat. The frontend's own
        # _cleanupIncognitoSessions handler knows which session is current and
        # won't delete the live one — this server-side purge exists only to
        # catch ghosts the frontend missed (tab close, crash). Only clean up
        # rows old enough to be definitely orphaned.
        #
        # Run at most once per 60s to avoid unnecessary DB round-trips.
        if _time.monotonic() - _last_incognito_cleanup_time > _INCOGNITO_CLEANUP_INTERVAL:
            _last_incognito_cleanup_time = _time.monotonic()
            try:
        ...
        # IMPORTANT: Filter by owner to avoid scanning ALL non-archived sessions
        # across all users — critical for multi-user scalability.
        db = SessionLocal()
        try:
            folder_map = {}
            token_map = {}
            ...
            if session_ids:
                rows = db.query(DbSession.id, DbSession.folder, DbSession.total_input_tokens, DbSession.total_output_tokens, DbSession.is_important, DbSession.created_at, DbSession.updated_at, DbSession.last_message_at, DbSession.mode, DbSession.message_count).filter(
