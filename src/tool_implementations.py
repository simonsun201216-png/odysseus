...
_OPENBB_BASE = "http://api-gw:28000/openbb"
_OPENBB_TIMEOUT = 30


async def _openbb_get(path: str, params: dict = None) -> dict:
    """Make an async GET request to the OpenBB API Gateway proxy."""
    import httpx as _httpx
    url = f"{_OPENBB_BASE}/{path.lstrip('/')}"
    try:
        async with _httpx.AsyncClient(timeout=_OPENBB_TIMEOUT) as client:
            resp = await client.get(url, params=params)
            resp.raise_for_status()
            return resp.json()
    except _httpx.TimeoutException:
        return {"error": f"OpenBB request timed out: {url}", "exit_code": 1}
    except _httpx.HTTPError as e:
        return {"error": f"OpenBB request failed: {str(e)[:200]}", "exit_code": 1}
    except Exception as e:
        return {"error": f"OpenBB unexpected error: {str(e)[:200]}", "exit_code": 1}


async def do_openbb_quote(content: str, owner: Optional[str] = None) -> Dict:
    ...
    if not symbol:
        return {"error": "symbol is required", "exit_code": 1}
    data = await _openbb_get("equity/price/quote", {"symbol": symbol, "provider": "yfinance"})
    results = data.get("results")
    if not results:
        return {"error": f"No quote data for {symbol}: {data.get('error') or data.get('detail', 'unknown')}", "exit_code": 1}
    lines = []
    for r in results:
        lines.append(f"{r.get('symbol', symbol)} ({r.get('name', '')})")
        lines.append(f"  Last: ${r.get('last_price', 'N/A')}  |  Open: ${r.get('open', 'N/A')}")
        lines.append(f"  High: ${r.get('high', 'N/A')}  |  Low: ${r.get('low', 'N/A')}")
        lines.append(f"  Volume: {r.get('volume', 'N/A'):,}  |  Avg Vol: {r.get('volume_average', 'N/A'):,}")
        lines.append(f"  Prev Close: ${r.get('prev_close', 'N/A')}  |  Change%: {r.get('change_percent', 0):.2f}%")
        lines.append(f"  Bid: ${r.get('bid', 'N/A')}  |  Ask: ${r.get('ask', 'N/A')}")
        lines.append(f"  50d MA: ${r.get('ma_50d', 'N/A')}  |  200d MA: ${r.get('ma_200d', 'N/A')}")
        lines.append(f"  52w High: ${r.get('year_high', 'N/A')}  |  52w Low: ${r.get('year_low', 'N/A')}")
        lines.append("")
    return {"output": "\n".join(lines).strip(), "exit_code": 0}


async def do_openbb_historical(content: str, owner: Optional[str] = None) -> Dict:
    ...
    data = await _openbb_get("equity/price/historical", params)
    results = data.get("results")
    if not results:
        return {"error": f"No historical data for {symbol}", "exit_code": 1}
    lines = [f"Historical prices for {symbol} ({len(results)} trading days):\n"]
    lines.append(f"{'Date':<14} {'Open':>10} {'High':>10} {'Low':>10} {'Close':>10} {'Volume':>12}")
    lines.append("-" * 70)
    for r in results[:30]:  # cap at 30 rows
        ...
        v = f"{r.get('volume', 0):,}"
        lines.append(f"{dt:<14} {o:>10} {h:>10} {l:>10} {c:>10} {v:>12}")
    if len(results) > 30:
        lines.append(f"\n... and {len(results) - 30} more rows. Use start_date/end_date to narrow the range.")
    return {"output": "\n".join(lines), "exit_code": 0}


async def do_openbb_fundamentals(content: str, owner: Optional[str] = None) -> Dict:
    ...
    if not symbol:
        return {"error": "symbol is required", "exit_code": 1}
    path_map = {"income": "equity/fundamental/income", "balance": "equity/fundamental/balance", "cash": "equity/fundamental/cash"}
    path = path_map.get(statement)
    if not path:
        return {"error": f"Invalid statement type: {statement}. Use: income, balance, cash", "exit_code": 1}
    data = await _openbb_get(path, {"symbol": symbol, "provider": "yfinance", "period": "annual"})
    results = data.get("results")
    if not results:
        return {"error": f"No {statement} data for {symbol}", "exit_code": 1}
    lines = [f"{symbol.upper()} — {statement.replace('_', ' ').title()} Statement\n"]
    r = results[0]
        ...
        if val is not None and val != "":
            if isinstance(val, float):
                ...
                if val >= 1_000_000_000:
                    lines.append(f"  {label:<50} ${val:,.0f}")
                elif val >= 1_000:
                    lines.append(f"  {label:<50} ${val:,.2f}")
                else:
                    lines.append(f"  {label:<50} {val}")
            else:
                lines.append(f"  {key.replace('_', ' ').title():<50} {val}")
    if len(results) > 1:
        lines.append(f"\n({len(results)} fiscal years available. Showing most recent.)")
    return {"output": "\n".join(lines), "exit_code": 0}


async def do_openbb_macro(content: str, owner: Optional[str] = None) -> Dict:
    ...
    symbol = args.get("symbol", "").strip().upper()
    if not symbol:
        return {"error": "symbol (FRED series ID) is required", "exit_code": 1}
    common_series = {
        ...
        "BAA10Y": "BAA Corporate Bond Yield - 10Y Treasury",
    }
    params = {"symbol": symbol, "provider": "fred"}
    if args.get("start_date"):
        params["start_date"] = args["start_date"]
    if args.get("end_date"):
        params["end_date"] = args["end_date"]
    data = await _openbb_get("economy/fred_series", params)
    results = data.get("results")
    if not results:
        # FRED may need an API key; try with yfinance for common indicators
        fallback_params = {"symbol": symbol, "provider": "yfinance"}
        if args.get("start_date"):
            fallback_params["start_date"] = args["start_date"]
        data = await _openbb_get("economy/fred_series", fallback_params)
        results = data.get("results")
    if not results:
        name = common_series.get(symbol, symbol)
        return {"error": f"Cannot fetch {name} ({symbol}). FRED requires an API key configured in the OpenBB platform settings.", "exit_code": 1}
    lines = [f"FRED Series: {common_series.get(symbol, symbol)} ({symbol})\n"]
    for r in results[:20]:
        dt = str(r.get("date", ""))[:10] if r.get("date") else ""
        val = r.get("value", "")
        lines.append(f"  {dt:<14} {val}")
    if len(results) > 20:
        lines.append(f"\n... and {len(results) - 20} more observations.")
    return {"output": "\n".join(lines), "exit_code": 0}


async def do_openbb_search(content: str, owner: Optional[str] = None) -> Dict:
    """Search for financial instruments/companies by symbol or name."""
    try:
        args = _parse_tool_args(content)
    except ValueError:
        return {"error": "Invalid JSON arguments", "exit_code": 1}
    ...
    if not query:
        return {"error": "query is required", "exit_code": 1}
    # yfinance doesn't have a keyword search endpoint; use profile lookup by symbol
    symbols = [s.strip().upper() for s in query.replace(",", " ").split() if s.strip()][:5]
    lines = [f"Results for '{query}':\n"]
    found = 0
    for sym in symbols:
        data = await _openbb_get("equity/profile", {"symbol": sym, "provider": "yfinance"})
        results = data.get("results")
        if results:
            r = results[0]
            name = r.get("name", r.get("long_name", ""))
            exch = r.get("exchange", "")
            sector = r.get("sector", "")
            industry = r.get("industry", "")
            mcap = r.get("market_cap", 0)
            mcap_str = f"${mcap:,.0f}" if mcap and isinstance(mcap, (int, float)) and mcap > 0 else ""
            parts = [f"  {sym:<8} {name}"]
            if exch: parts.append(f"({exch})")
            if sector: parts.append(f" [{sector}]")
            if mcap_str: parts.append(f" MktCap: {mcap_str}")
            lines.append(" ".join(parts))
            found += 1
        else:
            lines.append(f"  {sym:<8} (no data found)")
    if found == 0:
        return {"error": f"No results for '{query}'. Try using a ticker symbol (e.g. AAPL, MSFT, NVDA).", "exit_code": 1}
    return {"output": "\n".join(lines), "exit_code": 0}
