...
export async function loadSkills(cascade = false) {
  // One-time DOM event binding — runs on first load when this module is
  // dynamically imported from memory.js. Not on page load.
  if (!_eventsBound) {
    _eventsBound = true;
    _bindSkillsEvents();
  }
...
export async function loadDiscover(cascade) {
  try {
    const statusEl = document.getElementById('discover-scan-status');
    if (statusEl) statusEl.textContent = 'Loading…';
    const filter = document.getElementById('discover-filter')?.value || 'all';
    const categoryFilter = document.getElementById('discover-category-filter')?.value || 'all';
    const sortBy = document.getElementById('discover-sort')?.value || 'time';
    const params = new URLSearchParams({ limit: '500', sort_by: sortBy });
    if (filter !== 'all') params.set('status', filter);
    if (categoryFilter !== 'all') params.set('category', categoryFilter);
    const res = await fetch(`${API}/api/skills/discover/list?${params}`);
    const data = await res.json();
    const items = data.discovered || [];
    _renderDiscoverList(items);
    _updateDiscoverCount(items);
    if (statusEl) {
      statusEl.textContent = items.length ? `${items.length} results` : 'No discoveries yet. Click "Scan Now" to find skills.';
    }
  } catch (e) {
    console.error('Failed to load discoveries:', e);
    const list = document.getElementById('discover-list');
    if (list) list.innerHTML = '<div class="memory-empty">Failed to load discoveries.</div>';
  }
}

function _updateDiscoverCount(items) {
  const el = document.getElementById('discover-count');
  if (!el) return;
  const pending = items.filter(i => i.status === 'pending').length;
  el.textContent = pending || items.length || '0';
}

function _renderDiscoverList(items) {
  const list = document.getElementById('discover-list');
  if (!list) return;
  if (!items.length) {
    list.innerHTML = '<div class="memory-empty">No discoveries found. Run a scan to find new skills.</div>';
    return;
  }
  ...
  const CATEGORY_LABELS = {
    finance: '💰 Finance', dev: '💻 Dev', research: '🔬 Research',
    productivity: '⚡ Productivity', communication: '📨 Comms',
    data: '📊 Data', security: '🔒 Security', creative: '🎨 Creative',
    general: '📁 General',
  };

  let html = '';
  for (const item of filtered) {
    const t = item.name || 'unnamed';
    const desc = item.description || '(no description)';
    const repo = (item.source_repo || item.source_url || '').replace(/^https?:\/\//, '').slice(0, 60);
    const conf = item.relevance_score != null ? Math.round(item.relevance_score * 100) : null;
    const rep = item.source_reputation || 0;
    const sim = item.similarity_to_existing != null ? Math.round(item.similarity_to_existing * 100) : null;
    const matched = item.matched_to_skill || '';
    const statusClass = item.status || 'pending';
    const statusLabel = item.status === 'pending' ? 'Pending' : item.status === 'approved' ? 'Approved' : item.status === 'imported' ? 'Imported' : item.status === 'rejected' ? 'Rejected' : item.status;
    const catLabel = CATEGORY_LABELS[item.auto_category] || item.auto_category || '';
    const popScore = item.popularity_score || 0;
    // Show actions based on status
    let actions = '';
    if (item.status === 'pending') {
      actions = `
        <button class="memory-toolbar-btn discover-review-btn" data-id="${esc(item.id)}" data-decision="approve" title="Mark as approved">${_svg(_ICON.approve)} Approve</button>
        <button class="memory-toolbar-btn discover-review-btn" data-id="${esc(item.id)}" data-decision="reject" title="Reject this skill" style="opacity:0.6">✕ Reject</button>
        <button class="memory-toolbar-btn discover-import-btn" data-id="${esc(item.id)}" title="Import into skill library" style="color:var(--color-primary,#4f9)">${_svg(_ICON.approve)} Import</button>
      `;
    } else if (item.status === 'approved') {
      actions = `
        <button class="memory-toolbar-btn discover-import-btn" data-id="${esc(item.id)}" title="Import into skill library">${_svg(_ICON.approve)} Import</button>
        <button class="memory-toolbar-btn discover-del-btn" data-id="${esc(item.id)}" title="Remove from list" style="opacity:0.5">✕ Delete</button>
      `;
    } else if (item.status === 'imported') {
      actions = `<span style="font-size:0.85em;opacity:0.6;">✓ Imported</span>`;
    } else if (item.status === 'rejected') {
      actions = `<button class="memory-toolbar-btn discover-del-btn" data-id="${esc(item.id)}" title="Remove from list" style="opacity:0.5">✕ Delete</button>`;
    }
    html += `
      <div class="doclib-card" style="display:flex;flex-direction:column;gap:6px;padding:12px;border-left:3px solid ${statusClass === 'pending' ? 'var(--color-warning,#f0ad4e)' : statusClass === 'approved' ? 'var(--color-primary,#4f9)' : statusClass === 'imported' ? 'var(--color-success,#5cb85c)' : 'var(--color-muted,#666)'};" data-discover-id="${esc(item.id)}">
        <div style="display:flex;align-items:center;gap:8px;">
          <strong style="flex:1;font-size:1em;">${esc(t)}</strong>
          ${catLabel ? `<span class="memory-cat-badge" style="font-size:0.75em;background:color-mix(in srgb, var(--color-primary,#4f9) 15%, transparent);">${esc(catLabel)}</span>` : ''}
          <span class="memory-cat-badge" style="font-size:0.75em;background:color-mix(in srgb, ${statusClass === 'pending' ? 'var(--color-warning,#f0ad4e)' : statusClass === 'imported' ? 'var(--color-success,#5cb85c)' : 'var(--color-primary,#4f9)'} 20%, transparent);">${esc(statusLabel)}</span>
          ${popScore > 0 ? `<span style="font-size:0.75em;opacity:0.6;" title="Downloads">🔥 ${popScore}</span>` : ''}
        </div>
        <div style="font-size:0.85em;opacity:0.75;line-height:1.4;">${esc(desc)}</div>
        <div style="display:flex;flex-wrap:wrap;gap:6px;font-size:0.8em;opacity:0.6;">
          ${repo ? `<span title="Source">📦 ${esc(repo)}</span>` : ''}
          ${conf != null ? `<span>Relevance: ${conf}%</span>` : ''}
          ${rep ? `<span>Reputation: ${rep}</span>` : ''}
          ${sim != null ? `<span>Similar: ${sim}%</span>` : ''}
          ${matched ? `<span>Matches: ${esc(matched)}</span>` : ''}
        </div>
  ...
  list.innerHTML = html || '<div class="memory-empty">No matches for current filter.</div>';
}

async function _discoverScan() {
  ...
  if (statusEl) statusEl.textContent = 'Scanning repositories…';
  try {
    const res = await fetch(`${API}/api/skills/discover/run`, { method: 'POST' });
    const data = await res.json();
    if (data.ok) {
      if (statusEl) statusEl.textContent = `Found ${data.discovered} new skills.`;
      uiModule.showToast(`Discovery complete: ${data.discovered} skills found`);
      await loadDiscover();
    } else {
    ...
    uiModule.showError('Discovery scan failed: ' + e.message);
  } finally {
    btn.disabled = false;
...
async function _discoverReview(id, decision) {
  const actionsEl = document.querySelector(`.doclib-card[data-discover-id="${CSS.escape(id)}"] .discover-review-btn`);
  const originalHtml = actionsEl?.innerHTML || '';
  try {
    const res = await fetch(`${API}/api/skills/discover/${encodeURIComponent(id)}/review`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    uiModule.showError('Review failed: ' + e.message);
  }
}

async function _discoverImport(id) {
  const card = document.querySelector(`.doclib-card[data-discover-id="${CSS.escape(id)}"]`);
  if (card) card.style.opacity = '0.6';
  try {
    const res = await fetch(`${API}/api/skills/discover/${encodeURIComponent(id)}/import`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({}),
    });
    const data = await res.json();
    if (data.ok) {
      uiModule.showToast(`Imported: ${data.name || id}`);
      await loadDiscover();
      // Refresh skills list in background so the new skill shows up
    ...
    uiModule.showError('Import failed: ' + e.message);
    if (card) card.style.opacity = '1';
  }
}

async function _discoverDelete(id) {
  if (!confirm('Remove this discovery record?')) return;
  try {
    const res = await fetch(`${API}/api/skills/discover/${encodeURIComponent(id)}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.ok) {
      await loadDiscover();
    } else {
...
async function _discoverImportAllApproved() {
  const btn = document.getElementById('discover-import-all-btn');
  if (!btn || btn.disabled) return;
  btn.disabled = true;
  btn.textContent = '⏳ Importing…';
  try {
    const res = await fetch(`${API}/api/skills/discover/list?status=approved&limit=500`);
    const data = await res.json();
    const approved = data.discovered || [];
    if (!approved.length) {
      uiModule.showToast('No approved skills to import.');
      btn.disabled = false;
      btn.textContent = 'Import Approved';
      return;
    }
    let imported = 0, failed = 0;
    for (const item of approved) {
      try {
        const r = await fetch(`${API}/api/skills/discover/${encodeURIComponent(item.id)}/import`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({}),
        });
    ...
    uiModule.showToast(`Imported ${imported}/${approved.length} approved skills${failed ? ` (${failed} failed)` : ''}`);
    await loadDiscover();
    if (typeof loadSkills === 'function') loadSkills();
  } catch (e) {
    console.error('Bulk import failed:', e);
    uiModule.showError('Bulk import failed: ' + e.message);
  } finally {
    btn.disabled = false;
    btn.textContent = 'Import Approved';
  }
...
function _bindSkillsEvents() {
  ...
  document.getElementById('discover-scan-btn')?.addEventListener('click', _discoverScan);
  document.getElementById('discover-import-all-btn')?.addEventListener('click', _discoverImportAllApproved);
  document.getElementById('discover-filter')?.addEventListener('change', loadDiscover);
  document.getElementById('discover-category-filter')?.addEventListener('change', loadDiscover);
  document.getElementById('discover-sort')?.addEventListener('change', loadDiscover);
  document.getElementById('discover-search')?.addEventListener('input', () => {
    clearTimeout(window._discoverSearchTimer);
    window._discoverSearchTimer = setTimeout(loadDiscover, 300);
  });
  document.getElementById('discover-list')?.addEventListener('click', (e) => {
    const btn = e.target.closest('button');
    if (!btn) return;
    const card = btn.closest('.doclib-card[data-discover-id]');
    if (!card) return;
    const id = card.dataset.discoverId;
    if (btn.classList.contains('discover-review-btn')) {
      const decision = btn.dataset.decision;
      _discoverReview(id, decision);
    } else if (btn.classList.contains('discover-import-btn')) {
      _discoverImport(id);
    } else if (btn.classList.contains('discover-del-btn')) {
      _discoverDelete(id);
    }
  });
...
export default { loadSkills, loadDiscover, openSkill };
