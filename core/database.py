...
class Session(TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        Index('ix_sessions_cleanup', 'name', 'created_at'),
        Index('ix_sessions_owner_archived', 'owner', 'archived'),
    )
...
class DiscoveredSkill(TimestampMixin, Base):
    """A skill discovered from external sources (GitHub, web search, etc.),
    pending user review before import. Tracks the full lifecycle from discovery
    through review/import/rejection."""
    __tablename__ = "discovered_skills"

    id               = Column(String, primary_key=True, index=True)
    owner            = Column(String, nullable=True, index=True)  # who discovered / will review
    source_url       = Column(String, nullable=False)             # direct URL to SKILL.md or repo
    source_repo      = Column(String, nullable=True)              # GitHub "org/repo" or None
    source_type      = Column(String, nullable=False, default="github")  # github | websearch | manual

    name             = Column(String, nullable=False)
    description      = Column(String, default="")
    raw_metadata     = Column(Text, nullable=True)                 # JSON: full parsed frontmatter

    # Category & classification
    category         = Column(String, nullable=True, index=True)   # original category from SKILL.md frontmatter
    auto_category    = Column(String, nullable=True, index=True)   # LLM-classified: finance | dev | research | productivity | communication | data | general

    # Scoring & dedup
    relevance_score  = Column(Integer, nullable=True)              # 0-100, how relevant to project
    source_reputation = Column(String, nullable=True)              # high | medium | low
    similarity_to_existing = Column(Integer, nullable=True)        # 0-100 Jaccard %
    matched_to_skill = Column(String, nullable=True)               # existing skill name if duplicate

    # Popularity
    popularity_score = Column(Integer, nullable=False, default=0)  # download/import count

    # Lifecycle
    status           = Column(String, nullable=False, default="new")  # new | reviewed | imported | rejected | duplicate
    review_decision  = Column(String, nullable=True)               # approve | reject
    review_note      = Column(String, nullable=True)               # user's review comment
    reviewed_by      = Column(String, nullable=True)
    reviewed_at      = Column(DateTime, nullable=True)

    imported_skill_name = Column(String, nullable=True)            # the slug it was imported as
    imported_at      = Column(DateTime, nullable=True)

    __table_args__ = (
        Index('ix_discovered_skills_owner_status', 'owner', 'status'),
        Index('ix_discovered_skills_source', 'source_url'),
        Index('ix_discovered_skills_auto_category', 'auto_category'),
    )


def _migrate_discovered_skills_columns():
    """Add category/auto_category/popularity_score columns to discovered_skills if missing."""
    import sqlite3
    db_path = DATABASE_URL.replace("sqlite:///", "")
    if not os.path.exists(db_path):
        return
    try:
        conn = sqlite3.connect(db_path)
        cursor = conn.execute("PRAGMA table_info(discovered_skills)")
        columns = [row[1] for row in cursor.fetchall()]
        if "category" not in columns:
            conn.execute("ALTER TABLE discovered_skills ADD COLUMN category TEXT")
            logging.getLogger(__name__).info("Migrated: added 'category' column to discovered_skills")
        if "auto_category" not in columns:
            conn.execute("ALTER TABLE discovered_skills ADD COLUMN auto_category TEXT")
            logging.getLogger(__name__).info("Migrated: added 'auto_category' column to discovered_skills")
        if "popularity_score" not in columns:
            conn.execute("ALTER TABLE discovered_skills ADD COLUMN popularity_score INTEGER DEFAULT 0")
            logging.getLogger(__name__).info("Migrated: added 'popularity_score' column to discovered_skills")
        conn.commit()
        conn.close()
    except Exception as e:
        logging.getLogger(__name__).warning(f"Migration discovered_skills columns failed: {e}")
...
def init_db():
    ...
    _migrate_discovered_skills_columns()
    ...
    _migrate_add_sessions_cleanup_index()
    _migrate_add_sessions_owner_archived_index()
...
def _migrate_add_sessions_cleanup_index():
    """Add composite index on sessions(name, created_at) for the incognito
    cleanup query. Idempotent thanks to CREATE INDEX IF NOT EXISTS."""
    import sqlite3
    db_path = DATABASE_URL.replace("sqlite:///", "")
    if not os.path.exists(db_path):
        return
    try:
        conn = sqlite3.connect(db_path)
        conn.execute("CREATE INDEX IF NOT EXISTS ix_sessions_cleanup ON sessions(name, created_at)")
        conn.commit()
        conn.close()
    except Exception as e:
        logging.getLogger(__name__).warning(f"sessions cleanup index migration failed: {e}")


def _migrate_add_sessions_owner_archived_index():
    """Add composite index on sessions(owner, archived) for efficient
    user-scoped session listing. Critical for multi-user scalability.
    ...
    try:
        conn = sqlite3.connect(db_path)
        conn.execute("CREATE INDEX IF NOT EXISTS ix_sessions_owner_archived ON sessions(owner, archived)")
        conn.commit()
        conn.close()
    except Exception as e:
        logging.getLogger(__name__).warning(f"sessions owner/archived index migration failed: {e}")
