...
def setup_chat_routes(
    ...
    async def chat_endpoint(request: Request, chat_request: ChatRequest) -> Dict[str, str]:
        ...
        # Hedge fund team analysis command
        hf_response = await chat_handler.handle_hedge_fund_command(sess, message, owner)
        if hf_response:
    ...
    async def chat_stream(request: Request):
        ...
        _is_prophetis_request = bool(_tool_intent and _tool_intent.category == "prophetis_agent")
        if _is_prophetis_request:
            _prophetis_workspace = os.path.realpath(
                os.path.join(os.path.dirname(__file__), "..", "prophetis_research")
            )
            if os.path.isdir(_prophetis_workspace):
                workspace = _prophetis_workspace
                logger.info("Prophetis request forcing workspace: %s", workspace)
        ...
        elif _is_prophetis_request and chat_mode != "agent":
            # Prophetis slash commands should always execute in agent mode,
            # even if the session was previously left in research or another mode.
            chat_mode = "agent"
            logger.info("Prophetis request forcing agent mode for this turn")
        ...
        # Hedge fund team analysis command (/hf or /hedge-fund)
        if message and isinstance(message, str):
            hf_response = await chat_handler.handle_hedge_fund_command(sess, message, owner)
            if hf_response:
                return JSONResponse({"response": hf_response})

        # Check for research_pending BEFORE mode persist overwrites it.
        # Prophetis requests must bypass the legacy deep-research path entirely.
        do_research = str(use_research).lower() == "true"
        if not do_research:
            if get_session_mode(session) == 'research_pending':
                do_research = True
                logger.info(f"Session {session} in research_pending — auto-triggering research")
        if _is_prophetis_request and do_research:
            logger.info("Prophetis request overriding legacy research toggle")
            do_research = False
        if _is_prophetis_request:
            logger.info("Prophetis request disabling default KB/RAG meeting injection")
            use_rag = "false"
        if _is_prophetis_request and isinstance(message, str):
            message = re.sub(r"^\s*/(?:pp|prophetis)\b\s*", "", message, flags=re.I).strip() or message
        ...
        _forced_relevant_tools = None
        _prophetis_required_tools = {
        ...
        if _is_prophetis_request:
            # Prophetis must win over the legacy research and generic agent flows.
            logger.info("Prophetis Agent Mode activated. Keeping full tool access enabled.")
            ...
            disabled_tools.discard("edit_file")
            disabled_tools.discard("grep")
            disabled_tools.discard("glob")
            disabled_tools.discard("ls")
            disabled_tools.discard("web_search")
            disabled_tools.discard("web_fetch")
            disabled_tools.add("trigger_research")
            _forced_relevant_tools = set(_prophetis_required_tools)
            _prophetis_instruction = (
                "**PROPHETIS RESEARCH MODE ACTIVATED**\n"
                "The user has triggered the Prophetis Research workflow.\n"
                "You MUST act as the Prophetis Agent.\n"
                "Your ACTIVE WORKSPACE is the Prophetis repository root.\n"
                "You MUST NOT call the legacy `trigger_research` tool or use the Deep Research sidebar workflow.\n"
                "You MUST NOT inspect or rely on `deep_research`, `rag`, `personal_docs`, or meeting databases for this workflow.\n"
                "You MUST follow this exact startup order before any substantive analysis:\n"
                "1. Read `AGENTS.md`.\n"
                "2. Read `START_HERE.md`.\n"
                "3. Read `loops/analysis-routing.md`.\n"
                "4. Read `loops/research-loop.md`.\n"
                "5. Inspect `cases/` and create a NEW case directory for this task.\n"
                "6. Immediately write draft `routing.json` and draft `research-package-manifest.json` into that case directory before extended web research.\n"
                "7. Then write the remaining required research package artifacts into that case directory.\n"
                "Required minimum artifacts for a full research task:\n"
                "- `routing.json`\n"
                "- `investment-memo.md`\n"
                "- `evidence-log.csv`\n"
                "- `case-notes.md`\n"
                "- `research-package-manifest.json`\n"
                "After reading the required startup files, do NOT spend multiple rounds browsing old cases unless one specific reference is necessary.\n"
                "Prefer filling the templates and writing draft artifacts early, then refine them with new evidence.\n"
                "Do NOT provide a normal final research report unless the case directory exists and at least the required files have been written.\n"
                "If case creation is blocked, stop and explicitly report the blocker instead of continuing with a generic report.\n"
                "Execute the workflow using these tools only as needed: bash, python, read_file, write_file, edit_file, grep, glob, ls, web_search, web_fetch."
            )
            ctx.messages.insert(len(ctx.preface), {"role": "system", "content": _prophetis_instruction})
        elif auto_escalated:
            if _tool_intent.category != "prophetis_agent":
                disabled_tools.update({
            ...
            if _is_prophetis_request:
                for _msg in reversed(messages):
                    if _msg.get("role") == "user" and isinstance(_msg.get("content"), str):
                        _msg["content"] += (
                            "\n\n[Prophetis workflow hard requirements:"
                            " work only inside the Prophetis workspace;"
                            " first read AGENTS.md, START_HERE.md, loops/analysis-routing.md, loops/research-loop.md;"
                            " then inspect cases/ and create a new case folder;"
                            " immediately write draft routing.json and research-package-manifest.json;"
                            " then write investment-memo.md, evidence-log.csv, case-notes.md;"
                            " do not use deep_research, rag, personal_docs, or meetings data;"
                            " do not spend multiple rounds browsing old cases;"
                            " if the case package is not created, do not output a generic report.]"
                        )
                        break
            ...
            if _is_image_generation_session(sess, owner=_user):
                ...
                try:
                    ...
                    async for chunk in stream_agent_loop(
                        ...
                        owner=_user,
                        relevant_tools=_forced_relevant_tools,
                        fallbacks=_fallback_candidates,
                        workspace=workspace or None,
                        bypass_public_tool_block=_is_prophetis_request,
                    ):
                        if chunk.startswith("data: ") and not chunk.startswith("data: [DONE]"):
                            try:
                                data = json.loads(chunk[6:])
