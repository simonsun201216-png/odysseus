...
def setup_cookbook_routes() -> APIRouter:
    ...
    def _auto_register_llm_endpoint(
        ...
        if remote:
            ...
            host = (
                ...
                if _docker_host_gateway_reachable()
                else "localhost"
            )
    ...
    def _maybe_sweep_orphans(tasks: list, state: dict) -> None:
        ...
        logger.info(
            f"_maybe_sweep_orphans: entered, last_ts={_last_orphan_sweep_ts[0]}"
        )
        now = _time.monotonic()
        if now - _last_orphan_sweep_ts[0] < _ORPHAN_SWEEP_MIN_INTERVAL_S:
            logger.info(
                f"_maybe_sweep_orphans: rate-limited, {now - _last_orphan_sweep_ts[0]:.1f}s since last"
            )
            return
