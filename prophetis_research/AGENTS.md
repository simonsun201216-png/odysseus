# Prophetis Agent Guide

This repository is the `Prophetis` research workspace. Prophetis is a named research
protocol, not a fictional personality, adviser, trade bot, or background
automation promise.

## Read Order

1. `Prophetis.md` for the wake word, identity boundary, and memory contract.
2. `OPERATING_CONTRACT.md` for the one-screen lazy-loading map.
3. `START_HERE.md` when the user asks how to start, what Prophetis covers, or wants prompt examples.
...
| user intent | do this |
| --- | --- |
| empty first prompt, `hi Prophetis`, `你好 Prophetis`, `Prophetis mode`, or onboarding request | Return a concise `START_HERE.md` summary before any research workflow. |
| `update Prophetis`, `Prophetis self-update`, `�?GitHub 拉最�?Prophetis` | Run `scripts/Prophetis_update.sh`. Do not run `scripts/check_updates.sh` first. |
| `Prophetis help`, `怎么�?Prophetis`, `Prophetis 能做什么`, `start here` | Return the layered Start Here card from `START_HERE.md`; keep it user-facing and concise. |
...
| `Prophetis, 看一�?X` | Treat as `quick_map`; route first, then answer with source notes and refresh triggers. |
...
| `现在能不能买/�?X`, `能不能加/�?�?抄底 X`, actionability ask | Run `data/marginal-buyer-payoff-bridge.md` before `data/actionability-risk-control.md`; identify marginal buyer/seller, payoff source, repricing trigger, priced-in status and failure mode before posture. |
| daily/weekly market report, market brief, 盘前简�? 收盘复盘, 日报, 周报 | Use `loops/market-briefing-loop.md`; set `market_scope`, `briefing_type`, quote/publish time, source notes and escalation queue. |
| `Prophetis, 研究 X` | Use `loops/research-loop.md` unless routing selects a narrower path. |
| `Prophetis, 更新 X` | Use `loops/monitoring-loop.md`; focus on incremental evidence and thesis impact. |
...
line that `Prophetis help` shows the full prompt menu.
...
- Keep user-specific views, watchlists, preferences, holdings, weights, and
  portfolio constraints in gitignored `private/` state by default. Tracked Prophetis
