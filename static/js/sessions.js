...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = window.__t ? window.__t('chat_title', 'Prophetis') : 'Prophetis';
...
export async function loadSessions() {
  try {
    ...
    // When embedded in Prophetis iframe via AiWorkbench, suppress the
    // auto-select fallback so the user always lands on a fresh initial
    // conversation instead of the most recent session. Flag is set by
    // init.js and consumed here (one-shot — won't affect subsequent
    // loadSessions() calls triggered by session list refreshes).
    if (window._odyProphetisFresh) {
      _skipAutoSelect = true;
      savedId = null;
      window._odyProphetisFresh = false;
    }
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : (window.__t ? window.__t('chat_title', 'Prophetis') : 'Prophetis');
    }
    ...
    // Prevent the first user turn in a brand-new session from being rendered
    // twice: once optimistically on send, and again by the immediate history replay.
    const _firstHistoryMsg = (!isOC && Array.isArray(msgHistory) && msgHistory.length === 1) ? msgHistory[0] : null;
    const _existingUserBubbles = chatHistory ? Array.from(chatHistory.querySelectorAll('.msg.msg-user')) : [];
    const _existingAiBubbles = chatHistory ? chatHistory.querySelectorAll('.msg.msg-ai').length : 0;
    const _existingOnlyFirstUser =
      isSameSession &&
      hasExistingBubbles &&
      _existingUserBubbles.length === 1 &&
      _existingAiBubbles === 0 &&
      _firstHistoryMsg &&
      _firstHistoryMsg.role === 'user';
    if (_existingOnlyFirstUser) {
      const _existingRaw = String(_existingUserBubbles[0]?.dataset?.raw || '').trim();
      const _historyRaw = String(_firstHistoryMsg.content || '').trim();
      if (_existingRaw && _historyRaw && _existingRaw === _historyRaw) {
        if (chatHistory) chatHistory.classList.remove('no-animate');
        return;
      }
    }
