...
def _get_owned_document(
    db, Document, doc_id: str, owner: Optional[str], active_only: bool = False
):
...
def _most_recent_owned_document(
    db, Document, owner: Optional[str], active_only: bool = False
):
    q = db.query(Document)
    if active_only:
        q = q.filter(Document.is_active == True)
...
async def do_tail_serve_output(content: str, owner: Optional[str] = None) -> Dict:
    ...
    inner = (
        f"if [ -s {shlex.quote(log_path)} ]; then {file_inner}; else {pane_inner}; fi"
    )
    if remote:
        _pf = f"-p {shlex.quote(str(sport))} " if sport and str(sport) != "22" else ""
        cmd = (
...
async def do_list_cached_models(content: str, owner: Optional[str] = None) -> Dict:
    ...
    async def _scan_one(
        ...
        try:
            async with httpx.AsyncClient(timeout=60) as client:
                resp = await client.get(
                    f"{_COOKBOOK_BASE}/api/model/cached", params=p, headers=headers
                )
                data = resp.json()
...
async def do_openbb_quote(content: str, owner: Optional[str] = None) -> Dict:
    ...
    data = await _openbb_get(
        "equity/price/quote", {"symbol": symbol, "provider": "yfinance"}
    )
    results = data.get("results")
    if not results:
        return {
            "error": f"No quote data for {symbol}: {data.get('error') or data.get('detail', 'unknown')}",
            "exit_code": 1,
        }
    lines = []
    for r in results:
        ...
        lines.append(
            f"  Last: ${r.get('last_price', 'N/A')}  |  Open: ${r.get('open', 'N/A')}"
        )
        lines.append(f"  High: ${r.get('high', 'N/A')}  |  Low: ${r.get('low', 'N/A')}")
        lines.append(
            f"  Volume: {r.get('volume', 'N/A'):,}  |  Avg Vol: {r.get('volume_average', 'N/A'):,}"
        )
        lines.append(
            f"  Prev Close: ${r.get('prev_close', 'N/A')}  |  Change%: {r.get('change_percent', 0):.2f}%"
        )
        lines.append(f"  Bid: ${r.get('bid', 'N/A')}  |  Ask: ${r.get('ask', 'N/A')}")
        lines.append(
            f"  50d MA: ${r.get('ma_50d', 'N/A')}  |  200d MA: ${r.get('ma_200d', 'N/A')}"
        )
        lines.append(
            f"  52w High: ${r.get('year_high', 'N/A')}  |  52w Low: ${r.get('year_low', 'N/A')}"
        )
        lines.append("")
...
async def do_openbb_historical(content: str, owner: Optional[str] = None) -> Dict:
    ...
    lines.append(
        f"{'Date':<14} {'Open':>10} {'High':>10} {'Low':>10} {'Close':>10} {'Volume':>12}"
    )
    lines.append("-" * 70)
...
async def do_openbb_fundamentals(content: str, owner: Optional[str] = None) -> Dict:
    ...
    if not path:
        return {
            "error": f"Invalid statement type: {statement}. Use: income, balance, cash",
            "exit_code": 1,
        }
    data = await _openbb_get(
        path, {"symbol": symbol, "provider": "yfinance", "period": "annual"}
    )
    results = data.get("results")
    if not results:
...
async def do_openbb_macro(content: str, owner: Optional[str] = None) -> Dict:
    ...
    if not results:
        ...
        return {
            "error": f"Cannot fetch {name} ({symbol}). FRED requires an API key configured in the OpenBB platform settings.",
            "exit_code": 1,
        }
...
async def do_openbb_search(content: str, owner: Optional[str] = None) -> Dict:
    ...
    symbols = [s.strip().upper() for s in query.replace(",", " ").split() if s.strip()][
        :5
    ]
    ...
    for sym in symbols:
        data = await _openbb_get(
            "equity/profile", {"symbol": sym, "provider": "yfinance"}
        )
        results = data.get("results")
        if results:
            r = results[0]
            ...
            if mcap_str:
                parts.append(f" MktCap: {mcap_str}")
            lines.append(" ".join(parts))
            found += 1
        else:
    ...
    if found == 0:
        return {
            "error": f"No results for '{query}'. Try using a ticker symbol (e.g. AAPL, MSFT, NVDA).",
            "exit_code": 1,
        }
    return {"output": "\n".join(lines), "exit_code": 0}
