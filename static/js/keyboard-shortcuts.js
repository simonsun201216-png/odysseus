...
export function initKeyboardShortcuts(modules) {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (_matchesCombo(e, kb.search)) {
      e.preventDefault();
      const gs = globalSearchModule || window.globalSearchModule;
      if (gs) {
        gs.isOpen ? gs.toggle() : gs.openSearch();
    ...
    // "/" key opens global search (unless focused on an input/textarea)
    if (e.key === '/' && !e.ctrlKey && !e.metaKey && !e.altKey) {
      const tag = e.target.tagName;
      const isInput = tag === 'INPUT' || tag === 'TEXTAREA' || e.target.isContentEditable;
      if (!isInput) {
        e.preventDefault();
        const gs = window.globalSearchModule || globalSearchModule;
        if (gs) gs.openSearch();
        return;
      }
    }
      ...
      uiModule.styledConfirm('Delete this session?', { confirmText: 'Delete', danger: true }).then(ok => {
        ...
        fetch(`${API_BASE}/api/session/${sid}`, { method: 'DELETE' }).then(async () => {
          ...
          if (nextSession) {
            ...
            el('current-meta').textContent = window.__t ? window.__t('chat_title', 'Prophetis') : 'Prophetis';
