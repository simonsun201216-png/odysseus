...
def setup_skills_routes(skills_manager: SkillsManager) -> APIRouter:
    ...
    # Lazy-init the discovery service on first use (avoids import errors at
    # app startup when the DB may not yet have its tables).
    _discovery_service = None

    def _get_discovery_service():
        nonlocal _discovery_service
        if _discovery_service is None:
            from services.skills_discovery import SkillsDiscoveryService
            _discovery_service = SkillsDiscoveryService()
        return _discovery_service

    @router.post("/discover/run")
    async def run_discovery(request: Request):
        """Run the skills discovery pipeline: crawl known GitHub repos, parse
        SKILL.md files, score relevance, detect duplicates, store results."""
        import asyncio as _asyncio
        user = _owner(request)
        ...
        existing = skills_manager.load(owner=user)

        def _run():
            return svc.run_discovery(
                owner=user,
    ...
    async def list_discovered(request: Request):
        """List previously discovered skills, optionally filtered by status/category and sorted."""
        user = _owner(request)
        ...
        svc = _get_discovery_service()
        results = svc.list_discovered(
            owner=user, status=status, category=category,
            sort_by=sort_by, limit=limit,
        )
        return {"discovered": results, "count": len(results)}

    @router.post("/discover/{discovery_id}/review")
    async def review_discovered(request: Request, discovery_id: str):
        """Mark a discovered skill as approved or rejected."""
        user = _owner(request) or ""
        body = await request.json()
        ...
        svc = _get_discovery_service()
        ok = svc.review_discovered(discovery_id, user, decision, note)
        if not ok:
            raise HTTPException(404, "Discovery record not found or already imported")
        return {"ok": True, "decision": decision}

    @router.post("/discover/{discovery_id}/import")
    async def import_discovered(request: Request, discovery_id: str):
        """Import a discovered skill into the user's skill library.
        High-reputation sources auto-publish; other sources create drafts."""
        user = _owner(request) or ""
        body = {}
        try:
            body = await request.json() if request.headers.get("content-type", "").startswith("application/json") else {}
        ...
        svc = _get_discovery_service()
        result = svc.import_skill(discovery_id, skills_manager, owner=user, publish=publish)
        if result is None:
            raise HTTPException(400, "Could not import skill (not found, already imported, or parse failed)")
        return {
            "ok": True,
            "skill": result,
            "name": result.get("name"),
            "status": result.get("status"),
            "deduped": bool(result.get("_deduped")),
        }

    @router.delete("/discover/{discovery_id}")
    async def delete_discovered(request: Request, discovery_id: str):
        """Delete a discovery record."""
        user = _owner(request) or ""
        svc = _get_discovery_service()
        ok = svc.delete_discovered(discovery_id, user)
        if not ok:
            raise HTTPException(404, "Discovery record not found")
        return {"ok": True}
