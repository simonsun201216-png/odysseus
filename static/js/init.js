// Prophetis AI Workbench — Initialization Scripts
// ES6 module — extracted from index.html inline scripts

/* ── Prophetis user identity fetch interceptor ──
   When embedded as an iframe in Prophetis, the parent window sets
   window.__prophetisUser BEFORE the iframe loads. The Odysseus backend
   needs to know which Prophetis user is making each API request so it can
   isolate sessions/data by owner. We wrap the global fetch() to inject
   the user's email as an X-Prophetis-User-Email header on every request. */
(function() {
  if (typeof window === 'undefined' || typeof window.fetch !== 'function') return;
  var _isIframe = window.parent !== window;
  console.log('[init] fetch wrapper starting, isIframe=' + _isIframe + ', parent.__prophetisUser=', _isIframe ? window.parent.__prophetisUser : 'n/a');
  const _origFetch = window.fetch;
  window.fetch = function(input, init) {
    init = init || {};
    var _pUser = null;
    try {
      _pUser = (window.parent !== window && window.parent.__prophetisUser) || window.__prophetisUser;
    } catch (_) { /* cross-origin or no parent — ignore */ }
    var url = typeof input === 'string' ? input : (input && input.url ? input.url : '?');
    if (_pUser && _pUser.email) {
      var _headers = init.headers || {};
      if (_headers instanceof Headers) {
        _headers.set('X-Prophetis-User-Email', _pUser.email);
      } else {
        _headers['X-Prophetis-User-Email'] = _pUser.email;
      }
      init.headers = _headers;
      console.log('[init] fetch +header for', url, '→', _pUser.email);
    } else {
      console.log('[init] fetch NO header for', url, '_pUser=', _pUser);
    }
...
// When embedded in Prophetis iframe via AiWorkbench, always start with a
// fresh customized initial conversation instead of restoring the previous
// session. Clearing lastSessionId + the session-active flag ensures
// loadSessions() in sessions.js treats every entry as a "first load" and
// creates a new default-chat session rather than auto-selecting the most
// recent real session (its fallback when no targetId is found).
if (window.__prophetisUser) {
  Storage.remove('lastSessionId');
  sessionStorage.removeItem('ody-session-active');
  // One-shot flag consumed by sessions.js:loadSessions() to suppress the
  // _realSessions[0] auto-select fallback when entering from Prophetis.
  window._odyProphetisFresh = true;
...
/* ── Prophetis user identity ──
   When embedded as an iframe in the Prophetis frontend, update the sidebar
   user bar with the parent application's logged-in user info. This ensures
   the Settings > Account page and the sidebar user badge show the same
   identity the user sees in the rest of Prophetis. */
if (window.__prophetisUser) {
  var _pUser = window.__prophetisUser;
  var _nameEl = document.getElementById('user-bar-name');
  var _avatarEl = document.getElementById('user-bar-avatar');
  var _displayName = _pUser.name || _pUser.email || 'User';
  if (_nameEl) _nameEl.textContent = _displayName;
  if (_avatarEl) {
    _avatarEl.textContent = _displayName[0].toUpperCase();
  }
}
