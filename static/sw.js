...
//   - JS/CSS (/static/*.js|.css): cache-first + background refresh.
//     Versioned (?v=) files are immutable at the HTTP level; non-versioned
//     files have a 5min browser cache. SW cache serves instantly regardless.
...
self.addEventListener('fetch', (e) => {
  ...
  // JS/CSS: cache-first with background refresh.
  // Versioned (?v=) files are immutable at the HTTP level (max-age=1y);
  // non-versioned files have a 5min browser cache. In both cases the SW
  // cache serves instantly, with a silent background refresh so the next
  // visit picks up any updates.
