...
// Dynamically imported for meeting detail linking
let _meetingDetailModule = null;
async function _linkCitations(el) {
  if (!_meetingDetailModule) {
    try { _meetingDetailModule = await import('./meetingDetail.js'); } catch (_) { return; }
  }
  _meetingDetailModule.linkSourceCitations(el);
}
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = window.__t ? window.__t('role_ai', 'Prophetis') : 'Prophetis';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    if (role === 'assistant' && metadata && metadata.tool_events && metadata.tool_events.length > 0) {
      ...
      // Link citations in all AI message bodies
      setTimeout(() => {
        box.querySelectorAll('.msg-ai .body').forEach(b => _linkCitations(b));
      }, 0);
      return lastWrap;
    }
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Prophetis' : shortModel(resolvedModel);
    ...
    // Post-process: link source_XXXXX citations to meeting detail modal
    if (role === 'assistant') {
      // Defer to avoid blocking render
      setTimeout(() => { _linkCitations(wrap); }, 0);
    }
