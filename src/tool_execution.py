...
async def execute_tool_block(
    ...
    progress_cb: Optional[Callable[[Dict], Awaitable[None]]] = None,
    workspace: Optional[str] = None,
    bypass_public_tool_block: bool = False,
) -> Tuple[str, Dict]:
    ...
    from src.tool_implementations import (
        ...
        do_app_api,
        do_openbb_quote, do_openbb_historical, do_openbb_fundamentals,
        do_openbb_macro, do_openbb_search,
    )
    ...
    if tool in _ADMIN_TOOLS and not bypass_public_tool_block and not _owner_is_admin(owner):
        desc = f"{tool}: BLOCKED"
    ...
    if is_public_blocked_tool(tool) and not bypass_public_tool_block and not _owner_is_admin(owner):
        desc = f"{tool}: BLOCKED"
        result = {
            "error": (
    ...
    elif tool == "openbb_quote":
        desc = "openbb_quote"
        result = await do_openbb_quote(content, owner=owner)
    elif tool == "openbb_historical":
        desc = "openbb_historical"
        result = await do_openbb_historical(content, owner=owner)
    elif tool == "openbb_fundamentals":
        desc = "openbb_fundamentals"
        result = await do_openbb_fundamentals(content, owner=owner)
    elif tool == "openbb_macro":
        desc = "openbb_macro"
        result = await do_openbb_macro(content, owner=owner)
    elif tool == "openbb_search":
        desc = "openbb_search"
        result = await do_openbb_search(content, owner=owner)
