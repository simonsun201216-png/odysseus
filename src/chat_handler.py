...
class ChatHandler:
    ...
    async def handle_hedge_fund_command(self, session, message: str, owner: str) -> Optional[str]:
        """Process /hf or /hedge-fund commands. Returns response string or None."""
        text = message.strip()
        query = None
        prefix_lower = text.lower()

        if prefix_lower.startswith("/hf "):
            query = text[4:].strip()
        elif prefix_lower.startswith("/hedge-fund "):
            query = text[12:].strip()
        elif prefix_lower in ("/hf", "/hedge-fund"):
            return (
                "## Hedge Fund Team Analysis\n\n"
                "Usage: `/hf <query>`\n"
                "Example: `/hf 分析 NVDA 近期走势和投资价值`\n\n"
                "This triggers a multi-agent team analysis:\n"
                "- Stanley (CIO) breaks down the query\n"
                "- 4 P0 analysts research in parallel (Jintao, Fundamental, Technical, Risk Manager)\n"
                "- Stanley synthesizes the final report"
            )
        ...
        if not model or not endpoint_url:
            return "No model configured for this session. Select a model first before using /hf."

        from src.orchestrator import run_team_analysis, SPECIALIST_DEFS

        session.add_message(ChatMessage("user", message))

        try:
            state = await run_team_analysis(
                query=query,
                owner=owner,
                endpoint_url=endpoint_url,
                model=model,
                language=None,  # auto-detect from query
                max_iterations=2,
            )
        except Exception as e:
            logger.exception("[hedge_fund] Analysis failed")
            err_msg = f"Hedge fund analysis failed: {e}"
            session.add_message(ChatMessage("assistant", err_msg))
        ...
        result = (
            f"## Hedge Fund Team Analysis\n\n"
            f"**Query:** {state.query}\n"
            f"**Language:** {state.language}\n"
            f"**Iterations:** {state.iteration}\n"
            f"**Analysts consulted:** {len(state.reports)}\n\n"
        )
        ...
        if state.reports:
            ...
            for sid, report in state.reports.items():
                name = SPECIALIST_DEFS.get(sid, {}).get("name", sid)
                summary = state.summaries.get(sid, "")
                result += f"**{name}** ({sid.upper()}):\n{summary[:2500]}\n\n"

        session.add_message(ChatMessage("assistant", result))
        from src.database import update_session_last_accessed
        update_session_last_accessed(session.id)
        self.session_manager.save_sessions()
        return result
