...
def setup_model_routes(model_discovery):
    ...
    # Makes Odysseus available as a drop-in LLM backend for the full project.
    # Uses the user's configured endpoint/model via resolve_endpoint().

    @router.post("/v1/chat/completions")
    async def chat_completions_v1(body: dict, request: Request):
        """OpenAI-compatible chat completions endpoint.

        Routes through Odysseus's LLM provider system using the user's
        default chat endpoint and fallback chain.

        Request format (OpenAI-compatible):
          POST /api/v1/chat/completions
          {"model": "...", "messages": [...], "stream": true/false, "temperature": 0.7}

        Returns OpenAI-compatible JSON or SSE stream.
        Supports cookie auth (browser) and Bearer token (external clients).
        """
        from src.auth_helpers import get_current_user as _gcu, effective_user as _eu
        from src.endpoint_resolver import resolve_endpoint as _resolve_endpoint
        from src.endpoint_resolver import resolve_chat_fallback_candidates as _chat_fallbacks
        from src.llm_core import llm_call_async, stream_llm

        owner = _eu(request) or _gcu(request) or ""
        ...
        # Build candidate chain: primary endpoint + fallbacks
        primary = _resolve_endpoint("default", owner=owner)
        fallbacks = _chat_fallbacks(owner=owner)

        candidates = []
        if primary and primary[0] and primary[1]:
            candidates.append(primary)
        candidates.extend(f for f in fallbacks if f[0] and f[1])

        if not candidates:
            raise HTTPException(503, "No LLM endpoint configured. Add a model provider in Settings > Models, or set a default model.")
        ...
        if stream:
            ...
            async def event_stream():
                async for chunk in stream_llm(url, model, messages,
        ...
        text = await llm_call_async(url, model, messages,
                                     temperature=temperature,
                                     max_tokens=max_tokens,
                                     headers=headers)
        return {
